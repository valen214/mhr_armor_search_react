...
function func(arg){
  ...
  let sharp_white_length = 50;
  let sharp_purple_length = 10;

  let hit = 50;
  params.sharpness_phy = 1.39;
  params.sharpness_elem = 1.25;
  let purple_hit_damage = expected_damage_with_crit({
    skills, params
  })
  params.sharpness_phy = 1.32;
  params.sharpness_elem = 1.15;
  let white_hit_damage = expected_damage_with_crit({
    skills, params
  })
 
  let total_damage = 0;
  total_damage += purple_hit_damage * sharp_purple_length;

  hit -= sharp_purple_length;

  if(hit > 0){
    let remaining_hit = Math.max(hit, 0);
    total_damage += remaining_hit * sharp_white_length;
  }
